...
module.exports = async (req, res) => {
    ...
    try {
        const { amount, coin_id, id } = req.query;
        ...
        if (!coin_id || coin_id <= 0) {
            ret.setCode(400);
            throw new Error('coin_id inválido');
        }
        ...
        const buy = (await knex.raw(query, args))[0];

        ret.addContent('buy', 'ok');
        ...
        if (err.sqlMessage) {
            ret.setCode(400);
            ret.addMessage(err.sqlMessage);
        } else {
            ret = require('../../../helpers/error-handler')(err, req.ret);
        }
